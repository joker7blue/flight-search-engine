...
import { FlightCardDetail } from "./flight-card-detail";
...
export function FlightCard({ flight }: { flight: FlightOffer }) {
  ...
  return (
    <Card className="w-full group p-6 pb-0 hover:shadow-lg hover:shadow-primary/10 transition-all duration-300 border-border/50 bg-gradient-to-br from-card to-card/80 overflow-hidden">
      <div className="">
          ...
          <div className="flex md:flex-col items-center md:items-end gap-4 md:gap-3 border-t md:border-t-0 md:border-l border-border/50 md:pl-6 pt-4 md:pt-0">
            <div className="flex-1 md:flex-none text-left md:text-right">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                &asymp; {flight.travelerPricings[0]?.price.total}{" "}
                {flight.price.currency} per person
              </p>
            </div>
            <Button
              size="lg"
              className="w-auto md:w-auto bg-gradient-to-r from-primary to-secondary hover:from-primary/90 hover:to-secondary/90 shadow-lg shadow-primary/30 group-hover:shadow-xl group-hover:shadow-primary/40 transition-all"
            >
              Select Flight
      ...
      {/* Expanded Details */}
      <div
        className={`border-t border-border/50 bg-gradient-to-br from-primary/5 to-secondary/5 overflow-hidden transition-all duration-500 ease-in-out ${
